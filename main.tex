...
A thin lens of focal length $f$ is described can be described as imposing a phase shift $u \mapsto u e^{-ikx^2/2f}$. Upon comparison with equation \eqref{eq:schrodinger_like_solution}, we see that this corresponds to the Hamiltonian
\begin{equation}
    \mathbf{h} = \begin{pmatrix}
        1 & 0 \\
        0 & 0
...
In a birefringent material, different polarizations gain different phases upon propagation. In particular, there is an orthogonal set of polarization vectors, which we denote as $\ket{0}$ and $\ket{1}$, such that diagonalize the transformation $U_{\phi_0,\phi_1}$ generated by the wave plate, in such a way that $U_{\phi_0,\phi_1}\ket{j} = e^{i\phi_j}\ket{j}$. That is to say that, in the $\{\ket{0}, \ket{1}\}$ basis,
$U_{\phi_0,\phi_1}$ has a matrix representation
\begin{equation}
    \overline{\mathbf{U}}_{\phi_0,\phi_1} = \begin{pmatrix}
    e^{i\phi_0} & 0 \\
    0 & e^{i\phi_1}
    \end{pmatrix}.
\end{equation}

In general, these axis are rotated with respect to the horizontal and vertical directions, that is, we have $\ket{0} = R_{\theta}\ket{h}$ and $\ket{1} = R_{\theta}\ket{v}$ where $R_\theta$ is the rotation operator, which, in the $\{\ket{h}, \ket{v}\}$ basis, is represented by a matrix
\begin{equation}
    \mathbf{R}_{\theta} = \begin{pmatrix}
        \cos \theta & -\sin \theta \\
        \sin \theta & \cos \theta
    \end{pmatrix}.
\end{equation}
One checks that $R_\theta^{-1} = R_\theta^\dagger = R_{-\theta}$.

Then, in the $\{\ket{h}, \ket{v}\}$ basis, $U_{\phi_0,\phi_1}$ has a matrix
\begin{equation}
    \mathbf{U}_{\phi_0,\phi_1} = \mathbf{R}_{\theta} \overline{\mathbf{U}}_{\phi_0,\phi_1} \mathbf{R}_{-\theta} = \frac{e^{i\phi_0}+e^{i\phi_1}}{2}\mathbf{1} + \frac{e^{i\phi_0}-e^{i\phi_1}}{2}\mathbf{S}_{2\theta}
\end{equation}
where $\mathbf{1}$ is the identity matrix and we have introduced
\begin{equation}
    \mathbf{S}_{\theta} = \begin{pmatrix}
        \cos \theta & \sin \theta \\
        \sin \theta & -\cos \theta
    \end{pmatrix}
\end{equation}

A useful decomposition of $\mathbf{S}_{\theta}$ is given by
\begin{equation}
    \mathbf{S}_{\theta} = \mathbf{R}_{\theta}\mathbf{Z} =\mathbf{Z}\mathbf{R}_{-\theta}
\end{equation}
where
\begin{equation}
    ...
    \end{pmatrix}
\end{equation}
is a Pauli matrix. This allows us to easily calculate the composition
\begin{equation}
    \mathbf{S}_{\theta_2}\mathbf{S}_{\theta_1} = \mathbf{R}_{\theta_2}\mathbf{Z}^2\mathbf{R}_{-\theta_1} = \mathbf{R}_{\theta_2-\theta_1}
\end{equation}

\subsection{Half-wave plate}
A half-wave plate is characterized by phases $\phi_0 = 0$ and $\phi_1 = \pi$. Therefore, its matrix in the $\{\ket{h}, \ket{v}\}$ basis is $\mathbf{U}_{\theta}^{\lambda/2} = \mathbf{S}_{2\theta}$.

\subsection{Quarter-wave plate}
A half-wave plate is characterized by phases $\phi_0 = -\pi/4$ and $\phi_1 = \pi/4$. Therefore, its matrix in the $\{\ket{h}, \ket{v}\}$ basis is $\mathbf{U}_{\theta}^{\lambda/4} = (\mathbf{1} -i \mathbf{S}_{2\theta})/\sqrt{2}$.

\section{Polaritmetry}

A typical polarimetry experiment is performed by measuring the power of a unknown state of light $\rho$ after it passing through a quarter wave plate with angle $\theta_1$, a half-wave plate with angle $\theta_2$, and a linear polarizer, which allows only horizontal light to pass. This gives us access to a measurement of the projector onto the state
\begin{equation}
    \begin{aligned}
        \ket{\psi_{\theta_1, \theta_2}} &= (U^{\lambda/4}_{\theta_1})^\dagger (U^{\lambda/2}_{\theta_2})^\dagger \ket{h} \\ &= \frac{1}{\sqrt{2}}\left[ S_{2{\theta_2}} + i R_{2(\theta_1-\theta_2)} \right] \ket{h} \\ &= \frac{1}{\sqrt{2}}\left[ \left( \cos 2\theta_2 + i \cos 2\Delta \theta \right) \ket{h} + \left( \sin 2\theta_2 + i \sin 2\Delta \theta \right) \ket{v} \right].
    \end{aligned}
\end{equation}

By changing $\theta_1$ and $\theta_2$, we may get all of the polarization states required for polarimetry:
\begin{equation}
    \begin{aligned}
        \ket{\psi_{0,0}} = e^{i\pi/4}\ket{h} \\
        \ket{\psi_{0,\pi/8}} = e^{i\pi/4}\ket{r} \\
        \ket{\psi_{0,\pi/4}} = e^{-i\pi/4}\ket{v} \\
        \ket{\psi_{0,3\pi/8}} = e^{i\pi/4}\ket{l} \\
        \ket{\psi_{\pi/4,3\pi/8}} = -e^{-i\pi/4}\ket{a} \\
        \ket{\psi_{\pi/4,\pi/8}} = e^{i\pi/4}\ket{d}
    \end{aligned}
\end{equation}
