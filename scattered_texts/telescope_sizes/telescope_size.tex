...
\begin{abstract}
We develop diffraction-limited design rules for a free-space optical link with finite transmitter and receiver apertures. An uncertainty-relation argument establishes the characteristic aperture scale $\sqrt{\lambda z}$, and a Gaussian-beam calculation gives the practical $98\%$ efficiency criterion $\sqrt{D_tD_r}>D_{98}\approx1.71\sqrt{\lambda z}$, where $D_t$ and $D_r$ are the transmitter and receiver diameters. We then show how a displaced $4f$ transmitting telescope independently controls beam size and wavefront curvature. Finally, numerical paraxial propagation through finite circular pupils quantifies the departure from the all-Gaussian approximation and identifies the practical beam size and alignment for available telescope pairs. For the proposed UFF--CBPF link at $\lambda=810$ nm over 6.8 km, $D_{98}\approx12.7$ cm; the numerical optimum shifts agree closely with the Gaussian prediction, while finite transmitter clipping can substantially reduce the attainable efficiency.
\end{abstract}
...
To calculate this trade-off analytically, we now make the all-Gaussian approximation: the field is assumed to remain Gaussian after passing through the transmitter aperture. This is accurate when transmitter clipping is small, but becomes only an approximation when the pupil substantially truncates the beam. Within this approximation, let $\eta_{\mathrm{tot}} = \eta(D_t,W_0)\eta(D_r,W)$ be the overall transmission efficiency due to clipping at the transmitter and receiver. The finite-pupil numerical calculation in Sec.~\ref{sec:telescope_clipping} removes this assumption.
...
We now show how a transmitting telescope can control both the spot size $W_0$ and the wavefront curvature $F_0$. Let a telescope be formed by two lenses of focal lengths $f_1$ and $f_2$. In the $(x,p)^T$ convention used above, the free-propagation matrix $P(z)$ and a thin lens are represented by
\begin{equation}
    P(z) = \begin{pmatrix} 1 & z \\ 0 & 1 \end{pmatrix}, \qquad
...
This discrepancy also has a direct uncertainty-relation interpretation. The all-Gaussian calculation assumes a minimum-uncertainty field, which can attain the equality in the diffraction width product. A hard transmitter pupil converts this field into a truncated Gaussian, which is no longer a minimum-uncertainty state and has an increased effective beam-quality factor. The resulting diffraction sidelobes further reduce the power collected by a finite receiver aperture. Although this argument explains why the all-Gaussian result is optimistic, the full spatial profile is needed to predict the efficiency quantitatively, motivating the numerical propagation.
